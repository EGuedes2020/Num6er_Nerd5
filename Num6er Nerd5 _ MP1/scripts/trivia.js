function main() {
    ...
    function resultado() {
        ...
        if (numL > 0 && numV >= 0 && Number.isInteger(numN)) { // se o input estiver certo
            //-->VAI BUSCAR A API
            fetch('https://numbersapi.p.rapidapi.com/'+ num.value +'/trivia?fragment=true&notfound=floor&json=true', options) //é só meter o número no input na url, por isso tenho a concatenação
            .then(data => data.json())
            .then(function(data) {
                    //
